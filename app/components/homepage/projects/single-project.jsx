...
const SingleProject = ({ project }) => {
  ...
  return (
    <div className='group w-full h-fit flex flex-col items-center justify-center relative cursor-text overflow-hidden px-3 md:px-8 py-[1.4rem] bg-gradient-to-r from-gray-100 to-gray-200 dark:bg-[linear-gradient(90deg,#281e57_0%,#201435_100%)] shadow-2xl rounded-lg border border-gray-300 dark:border-[#1a1443]'
    >
      ...
      <div className='flex flex-col items-center justify-between w-full h-full'>
        <h2 className='text-gray-800 dark:text-[#EFF3F4] not-italic font-semibold text-[1.525rem] leading-[110%] text-center capitalize'>
          {name}
        </h2>
        <div className="p-6">
          <Image
        ...
        <div className="flex items-center justify-between w-full">
          <Link
            href={demo}
            target='_blank'
            className="flex justify-center items-center w-10 h-10 rounded-full border-2 border-gray-400 dark:border-[#EFF3F4] text-gray-700 dark:text-[#EFF3F4] transition-all duration-300 hover:bg-violet-500 dark:hover:bg-[#231d4b] hover:text-white dark:hover:text-violet-600 hover:border-violet-600 dark:hover:border-[#0F0C41] hover:scale-110 decoration-clone cursor-pointer no-underline delay-[0.3s]">
            <FaPlay />
          </Link>
      ...
      <p className="absolute w-[90%] md:w-[85%] md:min-h-[150px] translate-x-[-110%] transition-transform duration-[0.9s] p-6 leading-[110%] rounded-[0_20px_20px_0] left-0 top-0 bg-gray-100 dark:bg-[#0f0b24] text-gray-800 dark:text-[#EFF3F4] translate-y-[25%] md:translate-y-[50%] group-hover:translate-x-[-2%] text-xs md:text-sm">
        {description}
      </p>
      <div className='group-hover:translate-x-0 absolute w-[140px] text-[0.8rem] flex justify-center gap-2 flex-col translate-x-full transition-transform duration-[0.5s] delay-[0.3s] p-[0.825rem] rounded-[10px_0_0_10px] right-0 bottom-4 bg-gray-100 dark:bg-[#0f0b24] text-gray-800 dark:text-[#EFF3F4]'>
        {tags.map((tag, id) => (
